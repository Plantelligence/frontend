...
export const RelatoriosPage = () => {
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    setExporting(true);
    try {
      await exportRelatoriosPDF(selectedId);
    } catch {
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {selectedId && relatorios.length > 0 && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 transition hover:bg-red-50 disabled:opacity-50 dark:border-red-800/50 dark:hover:bg-red-900/20"
                  title="Exportar relatórios em PDF"
                >
                  {exporting
                    ? <><i className="fa-solid fa-spinner fa-spin text-xs" /> Exportando...</>
                    : <><i className="fa-solid fa-file-pdf text-xs" /> Exportar PDF</>}
                </button>
              )}
              {!isReader && selectedId && (
                <button
