...
export const DS = {
  ...
  cardBg:      'rounded-2xl border border-token-border bg-surface shadow-token',
  cardInner:   'rounded-xl border border-token-border bg-surface-alt',
  cardHover:   'hover:border-token-border-strong transition-all',
  // Dividers
  divider:     'border-token-border',
  // Texto semântico
...
export const PageHeader = ({ label, title, subtitle, children }) => (
  <div className="mb-8">
    ...
    <h1 className="text-2xl sm:text-3xl font-bold text-stone-800 dark:text-stone-100 mb-2">{title}</h1>
    {subtitle && <p className="text-sm text-stone-600 dark:text-stone-400 max-w-2xl">{subtitle}</p>}
    {children}
  </div>
);
...
export const SectionHeader = ({ icon, title, count }) => (
  <div className="flex items-center gap-3 mb-5 pb-4 border-b border-stone-200 dark:border-stone-800/50">
    {icon && (
      <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-red-500/10">
        <i className={`fa-solid ${icon} text-red-500 dark:text-red-400 text-base`} />
      </div>
    )}
    <h2 className="text-base font-semibold text-stone-800 dark:text-stone-100 flex-1">{title}</h2>
    {count !== undefined && (
      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-stone-100 dark:bg-stone-800 text-[10px] font-bold text-stone-600 dark:text-stone-400">
        {count}
      </span>
    )}
  </div>
...
export const DashCard = ({ children, className = '', hover = true, padding = true }) => (
  <div className={`rounded-2xl border border-stone-200 dark:border-stone-800/60 bg-white dark:bg-stone-900/35 ${padding ? 'p-5' : ''} ${hover ? 'hover:border-stone-300 dark:hover:border-stone-700/80 transition-all' : ''} ${className}`}>
    {children}
  </div>
);
...
export const DashInnerCard = ({ children, className = '' }) => (
  <div className={`rounded-xl border border-stone-200 dark:border-stone-700/40 bg-stone-50 dark:bg-stone-800/40 p-3 ${className}`}>
    {children}
  </div>
);

// ── DashMetric — bloco de métrica (temperatura, umidade, etc.) ────────────────
export const DashMetric = ({ label, value, unit, range, color = 'text-red-500 dark:text-red-400' }) => (
  <div className="rounded-xl border border-stone-200 dark:border-stone-700/40 bg-stone-50 dark:bg-stone-800/40 p-3">
    ...
    {range && <p className="text-[10px] text-stone-500 dark:text-stone-600 mt-1">{range}</p>}
  </div>
);

// ── DashBadge — badge de status ───────────────────────────────────────────────
const badgeColors = {
  green:   'border-emerald-300 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400',
  amber:   'border-amber-300 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400',
  red:     'border-red-300 dark:border-red-500/30 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400',
  blue:    'border-blue-300 dark:border-blue-500/30 bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400',
  purple:  'border-purple-300 dark:border-purple-500/30 bg-purple-50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400',
  stone:   'border-stone-300 dark:border-stone-700 bg-stone-100 dark:bg-stone-800/60 text-stone-600 dark:text-stone-400',
};
...
export const DashStatCard = ({ icon, iconColor = 'text-red-500 dark:text-red-400', iconBg = 'bg-red-500/10', label, value, description }) => (
  <div className="rounded-2xl border border-stone-200 dark:border-stone-800/60 bg-white dark:bg-stone-900/35 p-5 hover:border-stone-300 dark:hover:border-stone-700/80 transition-all">
    {icon && (
...
const infoColors = {
  ...
  green: 'border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300',
  amber: 'border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300',
  red:   'border-red-200 dark:border-red-500/30 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-300',
};
...
export const DashEmptyState = ({ icon = 'fa-inbox', title, description, action }) => (
  <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
    <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-stone-100 dark:bg-stone-800/60 border border-stone-200 dark:border-stone-700/40">
      <i className={`fa-solid ${icon} text-xl text-stone-400 dark:text-stone-500`} />
    </div>
    {title && <p className="text-sm font-semibold text-stone-700 dark:text-stone-300">{title}</p>}
...
export const DashSelectCard = ({ selected, onClick, title, description, disabled }) => (
  <button
    ...
    className={`rounded-2xl border p-4 text-left transition-all w-full ${
      selected
        ? 'border-red-500/70 bg-red-50 dark:bg-red-500/8 ring-1 ring-red-500/40'
        : 'border-stone-200 dark:border-stone-800/60 bg-white dark:bg-stone-900/35 hover:border-stone-300 dark:hover:border-stone-700/60 hover:bg-stone-50 dark:hover:bg-stone-900/50'
