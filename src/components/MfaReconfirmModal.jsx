...
// useEffect: para focar o input e tratar tentativas excessivas
// useRef: mantém referência ao input de código sem re-renders
import React, { useEffect, useRef, useState } from 'react';
// api: instância Axios com interceptores JWT para as chamadas de verificação MFA
import api from '../api/client.js';
import { useAuthStore } from '../store/authStore.js';

// Número máximo de tentativas erradas antes de bloquear a ação
const MAX_ATTEMPTS = 5;

export const MfaReconfirmModal = ({ title, description, onConfirm, onCancel }) => {
  // Dados do usuário logado para exibir nome e e-mail no modal
  const user = useAuthStore((s) => s.user);
  // setSession: atualiza o authStore com os novos tokens após confirmação MFA
  const setSession = useAuthStore((s) => s.setSession);

  // method: "totp" | "email" — método escolhido para verificação
  const [method, setMethod]           = useState('totp');
  // Código digitado pelo usuário (6 dígitos numéricos)
  const [code, setCode]               = useState('');
  // Indica que a requisição de verificação está em andamento
  const [loading, setLoading]         = useState(false);
  // Mensagem de erro para exibir ao usuário
  const [error, setError]             = useState(null);
  // Contador de tentativas erradas para acionar o bloqueio
  const [attempts, setAttempts]       = useState(0);

  // estado do fluxo e-mail: controla as etapas do envio por e-mail
  // 'idle': aguardando; 'sending': enviando; 'ready': código enviado, aguarda input
  const [emailStep, setEmailStep]     = useState('idle');
  // ID do desafio gerado pelo backend para validar o código por e-mail
  const [challengeId, setChallengeId] = useState(null);
  // Código de debug retornado em ambiente de desenvolvimento
  const [emailDebug, setEmailDebug]   = useState(null);

  // Ref para o campo de código — usado para focar automaticamente
  ...
  // Chama o endpoint de início do desafio por e-mail e guarda o challengeId
  const requestEmailCode = async () => {
    setEmailStep('sending');
    setError(null);
    try {
      const res = await api.post('/auth/mfa-reconfirm/initiate');
      // challengeId identifica este desafio específico no backend
      setChallengeId(res.data.challengeId);
      // debugCode é o código em texto claro — só presente em ambiente de desenvolvimento
  ...
  // Troca o método e inicia o envio do código por e-mail imediatamente
  ...
  // ── Mudar para TOTP ──────────────────────────────────────────────────────────
  // Volta para o método TOTP do autenticador e limpa os dados do e-mail
  const switchToTotp = () => {
    setMethod('totp');
    setCode('');
    setError(null);
  ...
  // ── Verificar código ─────────────────────────────────────────────────────────
  // Envia o código ao backend e atualiza a sessão com os novos tokens (sca=now)
  const handleVerify = async (e) => {
    e.preventDefault();
    if (!code.trim() || loading || attempts >= MAX_ATTEMPTS) return;

    // e-mail: garante que o challenge foi criado antes de submeter
    ...
    try {
      ...
      // Inclui o challengeId apenas no fluxo de e-mail — TOTP não precisa
      ...
      // Isso permite que as próximas requisições passem na verificação require_recent_mfa
      setSession({ user: freshUser, tokens });

      setCode('');
      // Executa a ação crítica que estava aguardando a confirmação MFA
      onConfirm?.();
    } catch (err) {
      const msg = err?.response?.data?.detail || 'Código inválido. Tente novamente.';
      // Incrementa o contador para acionar o bloqueio após MAX_ATTEMPTS
  ...
  // O botão de confirmar só é habilitado quando o código tem 6 dígitos e a sessão é válida
  ...
  return (
    ...
    >
      {/* Fundo decorativo com gradiente âmbar para diferenciar do lock screen */}
      ...
      <div className="relative w-full max-w-sm px-4">
        <div className="rounded-3xl border border-stone-800/60 bg-stone-900/85 p-8 shadow-2xl backdrop-blur">
          ...
          {/* Avatar do usuário com iniciais — cor âmbar para indicar ação crítica */}
          ...
          {/* Cabeçalho de segurança com badge âmbar de ação crítica */}
          ...
          {/* ── Método TOTP ── */}
          {/* Formulário para o usuário inserir o código do app autenticador */}
          {method === 'totp' && (
            <form onSubmit={handleVerify} className="space-y-4">
              <div className="flex flex-col gap-1.5">
                ...
                {/* Input numérico com fonte mono para facilitar a leitura dos 6 dígitos */}
                <input
                  ref={inputRef}
                  type="text"
                  inputMode="numeric"
                  ...
                  maxLength={6}
                  value={code}
                  // Filtra para aceitar apenas dígitos, prevenindo letras ou símbolos
              ...
              {/* Mensagem de erro exibida após tentativas erradas */}
              {error && (
              ...
              {/* Botão de confirmação com spinner durante o carregamento */}
              <button
                type="submit"
                disabled={!canSubmit}
              ...
              {/* Opção de usar e-mail em vez do autenticador */}
              <button
                type="button"
          ...
          {/* ── Método e-mail ── */}
          {/* Fluxo alternativo: envia código para o e-mail do usuário */}
          {method === 'email' && (
            <div className="space-y-4">
              {/* Estado de envio: spinner enquanto o e-mail está sendo disparado */}
              {emailStep === 'sending' && (
              ...
              {/* Estado idle ou ready: formulário de inserção do código */}
              {(emailStep === 'idle' || emailStep === 'ready') && (
                <form onSubmit={handleVerify} className="space-y-4">
                  {/* Confirmação de envio com o e-mail de destino */}
                  {emailStep === 'ready' && (
                    <div className="rounded-xl border border-sky-500/20 bg-sky-500/8 px-3 py-2.5 text-xs text-sky-300 text-center">
                      ...
                      {/* Código de debug visível apenas em desenvolvimento para facilitar testes */}
                      {emailDebug && (
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    {/* Input de código com as mesmas configurações do TOTP */}
                    <input
                      ref={inputRef}
                      type="text"
                      inputMode="numeric"
                      ...
                      onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))}
                      placeholder="000000"
                      // Desabilitado até o código ser enviado (emailStep !== 'ready')
                      disabled={loading || attempts >= MAX_ATTEMPTS || emailStep !== 'ready'}
                      autoComplete="one-time-code"
                      className="w-full rounded-xl border border-stone-700/60 bg-stone-800/50 px-4 py-3 text-center text-xl font-mono tracking-[0.5em] text-stone-100 placeholder:text-stone-600 placeholder:tracking-[0.3em] outline-none transition focus:border-amber-500/60 focus:ring-1 focus:ring-amber-500/20 disabled:opacity-50"
                    />
                    {/* Opção de reenvio caso o e-mail não tenha chegado */}
                    {emailStep === 'ready' && (
                      <button
                        type="button"
                  ...
                  {/* Mensagem de erro após tentativas inválidas */}
                  {error && (
                    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2.5 text-xs text-red-300">
                      <i className="fa-solid fa-circle-exclamation mt-0.5 flex-shrink-0" />
                      <span>{error}</span>
          ...
          {/* Cancelar: permite que o usuário desista sem executar a ação */}
        ...
        {/* Rodapé de segurança e conformidade LGPD */}
